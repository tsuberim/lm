...
def prepare(
    ...
    if tok_path.exists():
        print(f"Tokenizer exists at {tok_path}, loading...")
        tok = BPETokenizer.load(tok_path)
    else:
        print(f"Training tokenizer on {tokenizer_sample_size:,} examples (vocab_size={vocab_size})...")
        texts = ds["train"].select(range(tokenizer_sample_size))["text"]
        tok = BPETokenizer()
        tok.train(texts, vocab_size=vocab_size)
        tok.save(tok_path)
        print(f"Tokenizer saved: vocab_size={tok.vocab_size}")
